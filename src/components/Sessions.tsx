...
import { SortArrow, SortButton } from "./SessionTableParts";
...
export function Sessions({
  ...
  onSelect: (session: { id: string; source: string } | null) => void;
      ...
      <div className="panel">
        ...
        <LoadingOverlay active={loading && rows.length > 0} className="table-scroll">
          <table>
            <thead>
              <tr>
                <th className="session-expand-th">
            ...
            <tbody>
              {rows.map((row) => {
                ...
                return (
                  <SessionTableRow
                    ...
                    onToggle={() =>
                      onSelect(
                        isSelected ? null : { id: row.session_id, source: row.source },
                      )
                    }
                  />
                );
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={SESSION_TABLE_COLUMNS} className="analytics-empty">
                    {loading ? (
