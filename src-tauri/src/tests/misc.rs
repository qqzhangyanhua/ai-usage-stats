...
fn quota_row(
    provider: &str,
    application: &str,
    freshness: crate::domain::OfficialQuotaFreshness,
    windows: &[(&str, Option<f64>)],
    error: Option<&str>,
    ...
    crate::domain::OfficialQuotaRow {
        ...
        windows: windows
            .iter()
            .map(|(label, percent)| crate::domain::OfficialQuotaWindow {
                kind: (*label).into(),
                label: (*label).into(),
            ...
            .collect(),
        freshness,
        captured_at: Some("2026-08-22T12:00:00+00:00".into()),
        error: error.map(str::to_string),
    }
}

#[test]
fn tray_quota_lines_summarise_each_provider() {
    use crate::domain::OfficialQuotaFreshness::{Official, Stale, Unavailable};
    let quota = crate::domain::OfficialQuotaDto {
        rows: vec![
            quota_row(
                ...
                "Cursor",
                Official,
                &[("总量", Some(12.0)), ("Auto", Some(0.0))],
                None,
            ),
            // 超过 3 个窗口的用 +N 带过，否则 Droid 一家就能把菜单撑爆。
            quota_row(
                "droid",
                "Droid",
                Official,
            ...
            quota_row("grok", "Grok", Stale, &[("周额度", Some(13.0))], None),
            // 没数字的行给原因，不要凭空消失。
            quota_row("claude", "Claude", Unavailable, &[], Some("尚未捕获")),
        ],
        alerts_enabled: true,
    ...
    let lines = crate::tray::quota_menu_lines(&quota);
    assert_eq!(lines[0], "Cursor  总量 12% · Auto 0%");
    assert_eq!(
        lines[1],
        "Droid  标准 5 小时 1% · 标准 周 3% · 标准 月 6% +1"
    );
    // 过期的行标个星号，和标题里的记号一致。
    assert_eq!(lines[2], "Grok  周额度 13%*");
    assert_eq!(lines[3], "Claude：尚未捕获");
}

#[test]
fn tray_quota_lines_skip_windows_without_a_percentage() {
    use crate::domain::OfficialQuotaFreshness::Official;
    let quota = crate::domain::OfficialQuotaDto {
        rows: vec![quota_row(
            "cursor",
            "Cursor",
            Official,
            &[("总量", None), ("Auto", Some(5.0))],
            None,
        )],
        alerts_enabled: true,
        stale_after_minutes: 10,
    ...
    assert_eq!(crate::tray::quota_menu_lines(&quota), ["Cursor  Auto 5%"]);
}
