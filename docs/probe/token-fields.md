...
| Claude Code | `~/.claude/projects/<dashed-cwd>/<session>.jsonl` | 是 | assistant `message.usage`：input_tokens / output_tokens / cache_read_input_tokens / cache_creation_input_tokens | 模型：`message.model`；项目：目录名解码；会话：`sessionId` 或文件名 | 同一 `message.id` 一条（优先 stop_reason，否则 output 更大）；全零跳过 |
