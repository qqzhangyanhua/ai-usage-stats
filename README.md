...
- 总览按来源展示 7 天滚动用量（累计 Token/费用与日均值，贴近周度限额心智模型，非官方配额）
- 总览独立展示 Claude / Codex / Cursor 官方额度（账号级已用百分比与重置时间，带新鲜度；与本机估计窗分开）
- 官方额度达到 80% / 100% 时各弹一次系统通知（按窗口重置去重，过期数据不弹）；菜单栏在今日花费旁显示最紧的官方百分比
- 总览模块（指标、计费窗、滚动用量、趋势、热力图、明细等）可在首页或设置页开关；额度区块内的 Codex、Cursor Agent 等来源也可单独配置显示
